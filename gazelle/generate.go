...
func generateCMakeConfigureRule(res *language.GenerateResult, configFile *CMakeConfiguredFile, args language.GenerateArgs) {
	...
	cmd := fmt.Sprintf(`
		...
		# Copy source files to build directory with correct relative paths
		for f in $(SRCS); do
			# Extract just the basename for CMakeLists.txt and template files
			basename_f=$$(basename "$$f")
			# For CMakeLists.txt, place it at the root
			if [ "$$basename_f" = "CMakeLists.txt" ]; then
				cp "$$f" "$$BUILD_DIR/CMakeLists.txt"
			else
				# For template files, preserve their relative structure
				rel_path="$$f"
				# Remove package prefix if present (e.g., "testdata/platform_hpp_project/")
				if [[ "$$f" == */* ]]; then
					# Find the deepest directory that contains CMakeLists.txt to determine package root
					pkg_dir=$$(dirname "$$f")
		...
		if [ -f "%s" ]; then
			...
			echo "Error: Generated file %s not found after cmake configure"
			exit 1
		fi
		
		# Clean up temporary build directory
		rm -rf $$BUILD_DIR
	`, configFile.OutputFile, filepath.Dir(configFile.OutputFile), configFile.OutputFile, configFile.OutputFile, configFile.OutputFile)
