...
func (l *cmakeLang) GenerateRules(args language.GenerateArgs) language.GenerateResult {
	...
	// For example, if your go.mod is `module example.com/gazelle-foreign-cc`,
	// and generate.go is in `gazelle-foreign-cc/gazelle/`, then
	// import "example.com/gazelle-foreign-cc/gazelle"
...
func (l *cmakeLang) UpdateRules(args language.UpdateArgs) language.UpdateResult {
	log.Printf("cmakeLang.UpdateRules: Called for package %s. File: %s", args.Rel, args.File.Path)
	...
	// For example, to delete a rule:
	// res.Del = append(res.Del, oldRule)

	// To update a rule (less common here, as GenerateRules usually handles current state):
	// oldRule.SetAttr("srcs", newSources)
	// res.Updated = append(res.Updated, oldRule)
